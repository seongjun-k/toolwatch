...
# config의 vapid_private_key_file은 저장소 루트 기준 상대경로 — 실행 위치(cwd)와 무관하게
# 동작하도록 절대경로로 정규화한다. CONFIG[key]는 원본 상대경로 그대로 두고 별도 변수에
# 담는다 — save_config()가 CONFIG를 그대로 config.json에 되쓰므로, 여기서 CONFIG를
# 덮어쓰면 절대경로가 파일에 영구 저장돼 Linux(Pi5) 이관 시 Windows 경로 문자열이
# posix 상대경로로 오인되어 깨진다.
VAPID_KEY_PATH = str(ROOT_DIR / CONFIG["vapid_private_key_file"]) if CONFIG.get("vapid_private_key_file") else None
    ...
    try:
        for push_uid, body in pending:
            try:  # 푸시는 부가 기능 — 실패해도 판정 흐름에 영향 금지 (확장계획.md §E3)
                push.send_push(conn, push_uid, "toolwatch", body, CONFIG, VAPID_KEY_PATH)
